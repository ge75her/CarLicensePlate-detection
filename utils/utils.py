...
# get the center,h,w to identify the location of anchor box in differ type
    src_width = torch.unsqueeze(src_bbox[:, 2] - src_bbox[:, 0], -1)
    src_height = torch.unsqueeze(src_bbox[:, 3] - src_bbox[:, 1], -1)
    src_ctr_x = torch.unsqueeze(src_bbox[:, 0], -1) + 0.5 * src_width
    src_ctr_y = torch.unsqueeze(src_bbox[:, 1], -1) + 0.5 * src_height
# adjust the center values x,y,w,h
    dx = loc[:, 0::4]
    dy = loc[:, 1::4]
    dw = loc[:, 2::4]
    dh = loc[:, 3::4]
# linear regression
    ctr_x = dx * src_width + src_ctr_x # translation
    ctr_y = dy * src_height + src_ctr_y
    w = torch.exp(dw) * src_width # compression
    h = torch.exp(dh) * src_height
    dst_bbox = torch.zeros_like(loc)
    # change to pascal VOC
