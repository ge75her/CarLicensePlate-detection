...
class ProposalCreator():
    ...
    def __call__(self, loc, score,
        ...
        #  防止建议框超出图像边缘, use clamp to segment
...
class RegionProposalNetwork(nn.Module):
    ...
    def forward(self, x, img_size, scale=1.):
        ...
        #   回归预测对先验框进行调整
        #-----------------------------------------#
        rpn_locs = self.loc(x) #1,1 conv
        rpn_locs = rpn_locs.permute(0, 2, 3, 1).contiguous().view(n, -1, 4) #reshape
        #-----------------------------------------#
        #   分类预测先验框内部是否包含物体
        #-----------------------------------------#
        ...
        rpn_fg_scores = rpn_softmax_scores[:, :, 1].contiguous() # if anchor box obtain a object, the confidence score
